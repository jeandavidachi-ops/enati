/* Barre de recherche partagée du header (portée depuis public/js/versus-search.js).
   Dropdown live (groupes + tickers) -> navigation client vers /group/:id et /ticker/:address.
...
export default function VsSearch(props) {
        ...
        <div className="vs-search-drop">
          ...
          {res.groups.length > 0 && (
            <>
              ...
              {res.groups.map((g, i) => (
                <Link key={'g' + i} className="vs-search-row" to={'/group/' + g.group_id} onClick={() => setOpen(false)}>
          ...
          {res.tickers.length > 0 && (
            <>
              ...
              {res.tickers.map((t, i) => {
                ...
                return (
                  <Link key={'t' + i} className="vs-search-row"
                    to={'/ticker/' + encodeURIComponent(t.contract_address)} onClick={() => setOpen(false)}>
                    <span className="vs-search-av">
                      {imgUrl
