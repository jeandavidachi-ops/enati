/* Shared Versus auth widget — works on every page.
   Loaded as <script type="text/babel" src="/js/versus-auth.js"> BEFORE the page's
   own babel script. Exposes a global <AuthCorner/> that renders the header buttons
   (Sign In/Up -> Connect Telegram -> rank card) and its own modals via a portal. */
(function injectAuthStyles() {
  ...
  .vs-rc-btn{display:inline-flex;align-items:center;gap:12px;height:36px;padding:0 8px 0 6px;border-radius:9999px;
  ...
  .vs-rc-avatar{width:26px;height:26px;flex:0 0 26px;border-radius:50%;position:relative;overflow:hidden;
  ...
  .vs-rc-avatar .h{position:absolute;top:5px;left:6px;width:14px;height:11px;background:linear-gradient(#3d6cff,#1944cc);border-radius:5px 5px 1px 1px}
  .vs-rc-avatar .b{position:absolute;bottom:-4px;left:6px;width:15px;height:15px;background:#d8bd8f;border-radius:50%}
  .vs-rc-name{font-size:13px;font-weight:900;letter-spacing:-.3px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .vs-rc-stats{display:flex;align-items:center;gap:8px;font-size:12px;font-weight:900}
  .vs-rc-stats .vs-rc-line{width:1px;height:14px;background:rgba(255,255,255,.18)}
  .vs-rc-blue{color:#4ea1ff}.vs-rc-green{color:#37e44f}.vs-rc-red{color:#ff4f72}
  .vs-rc-arrow{width:26px;height:26px;flex:0 0 26px;border-radius:50%;display:grid;place-items:center;
    border:1.5px solid rgba(255,255,255,.45);color:#fff;font-size:16px;line-height:1;box-shadow:inset 0 0 10px rgba(255,255,255,.04);animation:vs-rc-arrow 2.6s ease-in-out infinite}
...
const { useState: vsUseState, useEffect: vsUseEffect } = React;
    ...
    <button className="vs-rc-btn" type="button" aria-label={"Open " + name + " profile"}>
      ...
      <div style={{ textAlign: "left", minWidth: 0 }}>
        ...
        <div className="vs-rc-stats">
          <span className="vs-rc-blue">48</span><span className="vs-rc-line"></span>
          <span className="vs-rc-green">31</span><span className="vs-rc-line"></span>
          <span className="vs-rc-red">17</span>
        </div>
      </div>
...
/* Drop-in for the header. Manages its own auth state + modals (portaled to body). */
function AuthCorner() {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {modalEl && ReactDOM.createPortal(modalEl, document.body)}
    </div>
  );
}
